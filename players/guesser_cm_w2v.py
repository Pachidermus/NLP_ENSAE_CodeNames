...
class GuesserW2V:
    """Guesser using W2V Google News embedding
    """   
    def __init__(self, model=None, printed_dist=True, max_dist=1):
        """Setup GuesserW2V details
    ...
    def _compute_distance(self, clue, board):
        ...
        """
         #  list all vocabulary words from w2v embedding
        vocab = list(self.model_vecs)   
        # capitalize if clue not in vocabulary
        if clue.lower not in vocab :
        ...
        for word in board:
            try:
                if word[0] == '*':
                    continue
                # in w2v google news embedding, capitalization matters, so we lowercase words and check if we need to capitalize them
                word2=word.lower()
                if word2 not in vocab :
                    word2 = word2.capitalize()
                # use cosine distance between each word of the board 
                w2v.append((self.model.distance(clue,word2),word))    
            except KeyError:
                continue
        w2v = list(sorted(w2v))
        return w2v
...
class CodemasterW2V:
    """Codemaster using W2V Google News embedding
    """  
    def __init__(self, model=None, red_team = True, nb_words_guess=None, vocab_size = 1000, team_w = 3, 
                 ennemy_w = 1, civilian_w = 1, assassin_w = 1, printed_dist=True, greed=0.1):
        """Setup CodemasterW2V details
        ...
        Args:
            ...
            nb_words_guess (int from 1 to 3, optional): 
                ...
                best number of words to be guessed according to a given greed threshold
    ...
    def get_clue(self):
        ...
        # cm_wordlist : take most common words from w2v google news embedding vocab
        ...
        for clue_num in range(1,np.min([len(team_words)+1,5])):
            ...
            for team_word_comb in list(itertools.combinations(team_words, clue_num)):
                ...
                for val in self.cm_wordlist :
                    ...
                    # compute a weighted average distance associated with the specific word from cm wordlist, and specific combo
