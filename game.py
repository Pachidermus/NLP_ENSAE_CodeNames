...
class Game2:
    """Class that setups up game details and calls two Guesser/Codemaster pairs to play the game with both teams
    """
    ...
    def __init__(self, codemaster_red, guesser_red, codemaster_blue, guesser_blue,
        ...
        Args:
            codemaster_red (:class:`Codemaster`):
                Codemaster (spymaster in Codenames' rules) class that provides a clue to the red team.
            guesser_red (:class:`Guesser`):
                Guesser (field operative in Codenames' rules) class that guesses based on clue from the red codemaster.
            codemaster_blue (:class:`Codemaster`):
                Codemaster (spymaster in Codenames' rules) class that provides a clue to the blue team.
            guesser_blue (:class:`Guesser`):
                Guesser (field operative in Codenames' rules) class that guesses based on clue from the blue codemaster.
            seed (int or str, optional): 
                Value used to init random, "time" for time.time(). 
                Defaults to "time".
            do_print (bool, optional): 
        ...
        self.codemaster_red = codemaster_red(**cmr_kwargs)
        self.guesser_red = guesser_red(**gr_kwargs)
        self.codemaster_blue = codemaster_blue(**cmb_kwargs)
        self.guesser_blue = guesser_blue(**gb_kwargs)
    ...
    def _accept_guess(self, guess_index, red_turn = True):
        """Function that takes in an int index called guess to compare with the key grid
        CodeMaster will always win with Red and lose if Blue =/= 7 or Assassin == 1
        """
        ...
        if red_turn:
            ...
            ally_size = 8
            enemy_size = 7
            ally_condition = GameCondition.RED_WIN
            enemy_condition = GameCondition.BLUE_WIN
        else:
            ally_check = "Blue"
            enemy_check = "Red"
            ally_size = 7
            enemy_size = 8
            ally_condition = GameCondition.BLUE_WIN
            enemy_condition = GameCondition.RED_WIN
        
        if self.key_grid[guess_index] == ally_check:
            self.words_on_board[guess_index] = "*"+ally_check+"*"
            if self.words_on_board.count("*"+ally_check+"*") >= ally_size:
                return ally_condition
            return GameCondition.HIT_ALLY

        elif self.key_grid[guess_index] == enemy_check:
            self.words_on_board[guess_index] = "*"+enemy_check+"*"
            if self.words_on_board.count("*"+enemy_check+"*") >= enemy_size:
                return enemy_condition
            else:
                return GameCondition.CONTINUE
    ...
    def write_results(self, num_of_turns):
        ...
        with open("results/bot_results.txt", "a") as f:
            f.write(
                f'TOTAL:{num_of_turns} B:{blue_result} C:{civ_result} A:{assa_result}'
                f' R:{red_result} RED CM:{type(self.codemaster_red).__name__} '
                f'BLUE CM:{type(self.codemaster_blue).__name__} '
                f'RED GUESSER:{type(self.guesser_red).__name__} '
                f'BLUE GUESSER:{type(self.guesser_blue).__name__} SEED:{self.seed}\n'
            )
        ...
        with open("results/bot_results_new_style.txt", "a") as f:
            results = {"game_name": self.game_name,
                       ...
                       "total_turns": num_of_turns,
                       "R": red_result, "B": blue_result, "C": civ_result, "A": assa_result,
                       "codemaster_red": type(self.codemaster_red).__name__,
                       "codemaster_blue": type(self.codemaster_blue).__name__,
                       "guesser_red": type(self.guesser_red).__name__,
                       "guesser_blue": type(self.guesser_blue).__name__,
                       "seed": self.seed,
                       "time_s": (self.game_end_time - self.game_start_time),
    ...
    def run(self):
        ...
        while game_condition != GameCondition.BLUE_WIN and game_condition != GameCondition.RED_WIN:
            ...
            game_condition = GameCondition.HIT_ALLY
            while guess_num <= clue_num and keep_guessing and game_condition == GameCondition.HIT_ALLY:
                ...
                if guess_answer is None or guess_answer == "no comparisons":
                    break
                guess_answer_index = words_in_play.index(guess_answer.upper().strip())
                game_condition = self._accept_guess(guess_answer_index, red_turn)

                if game_condition == GameCondition.HIT_ALLY:
                    print('\n' * 2)
                    self._display_board_codemaster()
                    guess_num += 1
                ...
                # if guesser selected a civilian or a blue-paired word
                    ...
                    #game_counter = 25
                    self._display_board_codemaster()
                    print("THE ASSASSIN HAS BEEN CALLED!")
                    if red_turn:
                        print("BLUE TEAM WINS")
                        self.winner = "Blue"
                        game_condition = GameCondition.BLUE_WIN
                    else:
                        print("RED TEAM WINS")
                        self.winner = "Red"
                        game_condition = GameCondition.RED_WIN
                    print("Game Counter:", game_counter)
                    if self.do_log:
                        self.write_results(game_counter)
                
                elif game_condition == GameCondition.BLUE_WIN:
                    self.game_end_time = time.time()
                    self._display_board_codemaster()
                    print("BLUE TEAM WINS")
                    self.winner = "Blue"
                ...
                elif game_condition == GameCondition.RED_WIN:
                    self.game_end_time = time.time()
                    self._display_board_codemaster()
                    print("RED TEAM WINS")
                    self.winner = "Red"
